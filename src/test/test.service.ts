...
export class TestService {
    ...
    async calculateFatRate(dto: FatRateCalculateDto, user: User){
        try {
            ...
            if(dto.gender === GenderEnum.Male){
                // erkek
                result = 495 / (1.0324 - 0.19077 * Math.log10(dto.waist - dto.neck) + 0.15456 * Math.log10(dto.height)) - 450;
            } else {
                result = 495 / (1.29579 - 0.35004 * Math.log10(dto.waist + dto.hip - dto.neck) + 0.22100 * Math.log10(dto.height)) - 450;
            }
            if(result <= 0) {
                throw new HttpException("Bilgileriniz yanlış olmalı. Yağ oranınız 0'ın altında.",HttpStatus.BAD_REQUEST);
            }
            let suggestions = "";
            let description = "";
            ...
            await this.fatRateRepo.update({id: newFatRate.id}, {
    ...
    async calculateVki(dto: VkiCalculateDto, user: User) {
        try {
            ...
            if(result <= 0) {
                throw new HttpException("Bilgileriniz yanlış olmalı. Vücut Kitle İndeksiniz 0'ın altında.", HttpStatus.BAD_REQUEST)
            }
            if(result <= 0) {
                throw new HttpException("Bilgileriniz yanlış olmalı. Vücut Kitle İndeksiniz 40'ın üzerinde", HttpStatus.BAD_REQUEST)
            }
            ...
            await this.vkiRepo.update({id: newVki.id}, {
                testId: testResult.id
            });
